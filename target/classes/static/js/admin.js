...
function patent_submit(divId) {
    ...
    var authors = document.getElementById("inventor").value;
...
function pro_submit() {
    ...
    alert(pro_name+pro_url+pro_desc);
...
function photo_submit(divId) {
    ...
    if (fileType == 'jpg' || fileType == 'png' || fileType == 'bmp'){
        // 格式正确
        var file = $('#new-photo')[0].files[0]; // 图片文件
        var fileName = file.name; // 图片文件名
        // 构建form数据
        var imgdata = new FormData(); //图片数据
        ...
        $("#upload").ajaxSubmit({
            ...
            success:function (result) {
                if(result.success){
                    alert("upload success-0924");
                    window.location.href = data.url;
                }else{
                    alert("upload failure-0924");
                }
            },
            error:function (result) {
                alert("数据请求失败");
...
function deleteUsr(obj) {
    ...
    // var table = document.getElementById("usr-table");
    var $td = $(obj).parents('tr').children('td');
    var name = $td.eq(0).text();
    if(name == "admin"){
        alert("Do not try to do this :)");
        return ;
    }
    // alert(name);
    $.ajax({
